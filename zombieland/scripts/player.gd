...
var health = 3 
@onready var gunsfx: AudioStreamPlayer3D = $gunsfx
@onready var gunreload: AudioStreamPlayer3D = $gunreload
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("reload") : 
		...
		bullet_left = 25
	$head/Camera3D/Label.text = str(bullet_left) + " / 25"
	$head/Camera3D/Label2.text = "HP : " + str(health) + " / 3"
